...
from src.utils.config_loader import get_player_sound, get_player_spawn_position_config

def system_collision_player_enemy(world: esper.World, screen: pygame.Surface, player_entity: int) -> None:
    components = world.get_components(CSurface, CTransform, CTagEnemy)
    hunters = world.get_components(CSurface, CTransform, CTagHunter)
    
    pl_st = world.component_for_entity(player_entity, CPlayerState)
